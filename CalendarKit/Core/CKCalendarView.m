...
- (id)init
{
    ...
    if (self) {
        ...
        _timeZone = nil;
        _date = [NSDate date];
        _displayMode = CKCalendarViewModeWeek;
...
- (void)layoutSubviewsAnimated:(BOOL)animated
{
    ...
    CGRect wrapperFrame = [self bounds];
    wrapperFrame.size.height -= wrapperFrame.origin.y;
    [[self wrapper] setFrame:wrapperFrame animated:animated];
    ...
    if ([[self table] contentSize].height > 30)
    {
        ...
        tableFrame.size.height = [[UIScreen mainScreen] bounds].size.height-tableFrame.origin.y-64;
        if ([[self table] contentSize].height < tableFrame.size.height)
        {
        ...
        float height=[[self table] contentSize].height-1;
        float width=[self table].frame.size.width;
        [self bottomBorder].frame = CGRectMake(0, height, width, 1);
        [self bottomBorder].backgroundColor = [UIColor colorWithWhite:1 alpha:1];
        [[self table] addSubview:[self bottomBorder]];
...
- (void)_layoutCellsAnimated:(BOOL)animated
{
    ...
    if (animated)
    {
        [UIView transitionWithView:self duration:0.8 options:UIViewAnimationOptionLayoutSubviews animations:^{
